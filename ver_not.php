<?
session_start();header("Cache-Control: no-store, no-cache, must-revalidate");header("Cache-Control: post-check=0, pre-check=0", false);header("Pragma: no-cache");header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");require_once("config/config.php");require_once("include/clasesmysql.php");$miconexion = new DB_mysql ;$miconexion->conectar($basededatos, $host, $usuario_bd, $contrasena_bd);
...
$destino = "colegiocheca@gmail.com";



//$destino = "dominus_132@hotmail.com";
...
$asunto = "COMENTARIOS DESDE EL FORMULARIO WEB";
...
if($nom=="") { $tok=false; $m .= "Debe ingresar su nombre compelto<br>";}
...
if($mai != "") {
	...
	$m .= "Debe ingresar una direccion de e-mail valida<br>";
...
}else{ $tok=false; $m .= "Debe ingresar una direccion de e-mail<br>";}



if($com=="") { $tok=false; $m .= "Debe ingresar su consulta<br>";}
...
<title>::: Instituci&oacute;n Educativa Particular Rosa Mar&iacute;a Checa ::: Chiclayo - Congregaci&oacute;n del Buen Pastor</title>
  ...
  <tr>
    ...
    <td width="754" valign="top"><table width="754" border="0">
      <tr>
        ...
        <td width="748"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="748" height="259" title="ba2">

          <param name="movie" value="imagenes/ba2.swf" />

          <param name="quality" value="high" />

          <embed src="imagenes/ba2.swf" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="748" height="259"></embed>
		...
		<div align='center'>
			<?
		$not=$_GET['nt'];
		$miconexion->consulta("select * from noticia where not_id='$not'");
		$row_not1=mysql_fetch_array($miconexion->Consulta_ID);
		$not_1_id=$row_not1['not_id'];
		$not_1_ti=htmlentities($row_not1['not_nom']);
		$not_1_pie=htmlentities($row_not1['not_pie']);
		$not_1_de=$row_not1['not_desc'];
		$not_1_fe=explode('-',$row_not1['not_fecha']);
		$not_dia=$not_1_fe[2];
		$not_mes=$not_1_fe[1];
		$not_ano=$not_1_fe[0];
						...
						<table border='0' align='center' width='99%'>
							...
							<tr>
								<td align='left' style='padding-left:15px; padding-right:15px; padding-top:5px; padding-bottom:5px; font-size:10px; font-family:Verdana; color:#666666;'>
									<? echo "$semana, $not_dia de $mes_letra del $not_ano"; ?>
								</td>
							</tr>
							<tr>
								<td align='center' class='tit_not'>
									<?
									if(file_exists("noticia/$not_1_id.jpg"))
									{
									?>
										<img src='noticia/<?=$not_1_id?>.jpg' width='658' border='0' alt=''/>
									<?
									}
									else
									{
							...
							<tr>
								<td align='left'>
									<?
									if(file_exists("noticia/$arc_1") && $arc_1!='')
									{
									?>
										<table border='0' cellspacing='2' cellpadding='2'>
											<tr>
												...
												<td valign='top' style='border:1px solid #FFFFFF;'>
													<a href='noticia/<?=$arc_1?>' target='_blank' style='color:#990000; font-weight:bold; font-family:verdana; font-size:11px;'>
														Descargar
													</a>
												</td>
											</tr>
